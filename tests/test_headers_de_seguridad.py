...
def test_las_respuestas_traen_los_headers_de_seguridad(client):
    r = client.get("/health")
...
def test_es_la_politica_de_SPA_y_no_la_de_las_apps_Jinja(client):
...
def test_tambien_en_una_respuesta_de_error(client):
    ...
    r = client.get("/una-ruta-que-no-existe-en-ningun-producto")
    assert r.headers["Content-Security-Policy"] == CSP_SPA
